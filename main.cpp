...
string passGen(int user_number){
    ...
    srand(time(0));
    vector<string>randoms=words;
    random_shuffle(randoms.begin(),randoms.end());
    randoms.resize(user_number);
    string password="";

   randoms.resize(user_number);

    for(string word:randoms){
        password+=word;
...
//q2
string rep_with_upper(string string_value ){
    vector<char> values(string_value.begin(),string_value.end());
    char word=values[rand() % values.size()];
    string new_word="";

    for(int i=0; i < values.size();i++){
        if (values[i]==word){
            values[i]= toupper(word);
        }
    }
    new_word=string(values.begin(),values.end());
    return new_word;
}
string swapletters(string s){
    if(s.length()<2){
        ...
        return s.substr(s.length()-2,2)+s.substr(2,s.length()-4)+s.substr(0,2);
    }
}

//q3
bool search_letter(char letter, string word){
    for(char c :word){
    if(c==letter){
        return true;
    }
...
while (true){
        ...
        cout<<"Enter the number between 3 and 7 (inclusive):"<<endl;
        cin>>user_number;
        if (3<=user_number&& user_number<=7){
            break;
        }
        else{
            cout<<"The number must be suitable!!!"<<endl;
        }

    }
    string password= passGen(user_number);
    cout<<passGen(user_number)<<endl;
    cout<<rep_with_upper(password)<<endl;
    cout<<swapletters(rep_with_upper(password))<<endl;

    char letter;
    cout<<"Enter the searching letter:";
    cin>>letter;
    cout<< boolalpha<<search_letter(letter, passGen(user_number))<<endl;
    return 0;
